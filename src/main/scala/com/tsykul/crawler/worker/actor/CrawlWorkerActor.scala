...
class CrawlWorkerActor(val dispatcherActor: ActorRef) extends Actor with ActorLogging {

  import context.dispatcher

  override def receive: Receive = {
    case config@CrawlDefinition(_, _, _, crawlUid) =>
      val crawlRoot = context.actorOf(Props(classOf[CrawlRootActor], dispatcherActor), s"crawlRoot-$crawlUid")
      crawlRoot ! config
    case status@GetCrawlStatus(crawlUid, requester) =>
      log.debug(s"Got a status request for crawlUid: $crawlUid")
