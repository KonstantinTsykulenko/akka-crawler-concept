...
class Parser extends Actor with ActorLogging with HtmlParser {
  ...
  override def receive: Receive = {
    case CrawlUrlContents(SingleUrl(url, _,  filters, depth), resp, worker) =>
      log.debug(s"Parsing http response from $url")
      val links = parseHtml(resp)
      log.debug(s"links: $links")
      links.filter(link => filters.forall(link.matches)).foreach(link => worker ! Work(SingleUrl(link, Some(url), filters, depth)))
      worker ! NoMoreWork
